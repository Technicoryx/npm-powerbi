import {
  ...
  import { NpmPowerBiService } from './npm-powerbi.service';
  ...
  @Component({
    selector: 'npm-powerbi-component',
    template: '<div #npmPowerBiIFrame style="height:100%; width: 100%;"></div>',
    styles: []
  })
  export class NpmPowerBiComponent implements AfterViewInit, OnChanges, OnDestroy {
    // Public properties
    // Public properties
      @Input()
      accessToken!: string;
    ...
    @ViewChild('npmPowerBiIFrame', { static: true })
    npmPowerBiIFrameRef!: ElementRef;
  
    // Private fields
    // Private fields

      private component!: pbi.Embed;
    private powerBiService: NpmPowerBiService;
  
    constructor(private npmPowerBiService: NpmPowerBiService) {
      this.powerBiService = npmPowerBiService;
    }
  
    ngAfterViewInit() {
      // Embed the report inside the view child that we have fetched from the DOM
      if (
        this.npmPowerBiIFrameRef.nativeElement &&
        this.validateRequiredAttributes()
      ) {
        this.embed(this.npmPowerBiIFrameRef.nativeElement, this.getConfig());
      }
    }
  
    ngOnChanges(changes: SimpleChanges) {
      if (!this.npmPowerBiIFrameRef) {
        return;
      }
      const {
        accessToken,
      ...
      if (this.validateRequiredAttributes()) {
        ...
        this.embed(this.npmPowerBiIFrameRef.nativeElement, config);
      } else if (this.component) {
        this.reset(this.npmPowerBiIFrameRef.nativeElement);
      }
    }
